...
impl RunnerClient {
	...
	pub async fn initialize_scenario(&self, db_scenario_id: &str) -> Result<Scenario, Box<dyn Error>> {
		#[derive(Deserialize)]
		...
		let resp: InitializeScenarioResponse = self.post(&format!("/Scenarios/initialize_scenario/{}", db_scenario_id), &()).await?;
		Ok(resp.scenario)
	}

	/// Assigns vehicles to customers.
	/// Recommendation: fail if failed_to_update is not empty
	pub async fn update_scenario(&self, scenario_id: &str, update_vehicles: &UpdateScenario) -> Result<UpdateScenarioResponse, Box<dyn Error>> {
		let scenario: UpdateScenarioResponse = self.post(&format!("/Scenarios/update_scenario/{}", scenario_id), &()).await?;
		Ok(scenario)
	}

	/// Launches a scenario with a given speed
	pub async fn launch_scenario(&self, scenario_id: &str, speed: f64) -> Result<UpdateScenarioResponse, Box<dyn Error>> {
		#[derive(Serialize)]
		...
		let scenario: UpdateScenarioResponse = self.post(&format!("/Runner/launch_scenario/{}", scenario_id), &LaunchScenarioParameters{
			speed,
			scenario_id,
		}).await?;
		Ok(scenario)
